...
export class UsersService {
  ...
  async update(
    ...
    if (dto.roles !== undefined) {
      throw new BadRequestException(
        'Los roles se asignan mediante memberships',
      );
    }
  ...
  async restore(id: string): Promise<void> {
    ...
    await this.userRepository.update(id, { status: 'active', deletedAt: null });
  }
  ...
  async findAll(query: FilterUserDto = {}): Promise<UserDto[]> {
    ...
    const withRoles =
      query.withRoles === undefined ? true : this.toBoolean(query.withRoles);
    ...
    if (all) {
      ...
      usersQuery.where('user.status = :status', { status: 'active' });
    }

    if (shouldLoadRoles) {
      usersQuery.leftJoinAndSelect(
        'user.memberships',
        'membership',
        'membership.status = :membershipStatus',
        { membershipStatus: 'active' },
      );
      usersQuery.leftJoinAndSelect('membership.organization', 'organization');
      usersQuery.leftJoinAndSelect('membership.role', 'role');
      ...
      usersQuery.innerJoin('membership.role', 'role');
    }
    ...
    const users = await usersQuery.orderBy('user.createdAt', 'DESC').getMany();
  ...
  async updateLastLogin(userId: string, date: Date): Promise<void> {
    await this.userRepository.update(userId, { lastLoginAt: date });
  }
